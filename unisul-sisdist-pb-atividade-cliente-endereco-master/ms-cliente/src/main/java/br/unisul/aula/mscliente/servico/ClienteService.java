...
public class ClienteService {
    ...
    public List<ClienteDTO> listarTodos() {
        ...
        for (Cliente cliente: clienteRepository.findAll()){
            EnderecoDTO enderecoDTO = buscarEnderecoPorID(cliente.getEnderecoId());
    ...
    public CidadeClienteResponse listarCidades(String cidade){
        CidadeClienteResponse cidadeClienteResponse = new CidadeClienteResponse();
        List<EnderecoDTO> enderecoDTOList = enderecoClient.buscarPorCidade(cidade);
        cidadeClienteResponse.setCidade(enderecoDTOList.get(0).getCidade());
        cidadeClienteResponse.setUf(enderecoDTOList.get(0).getUf());
        for (EnderecoDTO enderecoDTO: enderecoDTOList){
            Cliente cliente = clienteRepository.findByEnderecoId(enderecoDTO.getId());
            cidadeClienteResponse.addCliente(cliente.getId(), cliente.getNome());
        }
        return cidadeClienteResponse;
    }
